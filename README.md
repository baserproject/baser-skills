...
| スキル | 概要 |
| --- | --- |
| [`basercms-plugin-migration`](skills/basercms-plugin-migration/SKILL.md) | baserCMS プラグインを 5.2系 → 5.3系 へ移行する際の固有の破壊的変更・非推奨・テスト基盤対応レシピ集 |
...
> これら4スキルは説明文の中で相互参照しているため、**まとめて導入する**ことを推奨します。

---

## 導入方法

利用する3つのツールいずれかで導入できます。**スキルはローカルにファイルが存在して初めて自動起動**します（リモート参照では起動しません）。利用側プロジェクトで一度インストールしてください。

以下の例は **常に最新版を取得**します（多くの用途ではこれで十分）。特定バージョンに固定したい場合は各ツールの「固定」注記を参照してください。

バージョンは [セマンティックバージョニング](https://semver.org/lang/ja/)（`v` プレフィックスなし。例 `1.0.0`）で管理しています。利用可能なバージョンは次で確認できます。

```bash
gh release list -R baserproject/baser-skills
# または: git ls-remote --tags https://github.com/baserproject/baser-skills
```
